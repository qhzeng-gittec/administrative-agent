...
| 当前进化流程 | 75项相关回归测试通过 | 验证调用协议、能力复用、候选评测、灰度和回滚；尚无该版本的真实模型能力增益测量 |
...
当前流程通过75项直接相关的离线回归测试，覆盖Skill／工具候选、现有能力复用、补充题结构与来源校验、留出隔离、judge未知或错误证据、非行政字段评审、失败续跑、灰度、正式启用与回滚。测试使用模型替身验证工程行为，没有重新调用付费模型测能力提升。
